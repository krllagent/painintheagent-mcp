...
`GET /api/v1/watermark/limits` returns `runs_used`, `runs_limit`, `max_chars`, `min_chars`, `period` and `shared_with_browser`. Only successful operations count toward the monthly allowance, while pending requests reserve slots. All four text tools share this allowance. Quotas and request throttles can reject a new run with HTTP 429. All operations use the same endpoint and confirmed account key.
...
Choose `operation`: `watermark` (default), `humanize`, `compare` or `detect-ai`. Only watermark accepts `slop_removal`; humanize accepts `watermark_removal` instead. Comparison requires `candidate_text` alongside the original `text`. AI detection requires at least 80 words of English or Russian. Each successful operation uses one shared run. The operation and its options are included in request identity.

`GET /api/v1/watermark/run?id=UUID` returns JSON with `status: "pending"`, `"success"` or `"error"`. A pending reply can include the last progress event. A success includes the text and relevant source notes or `ai_assessment`. Comparison preserves the supplied candidate. AI assessment returns the original text and quoted style evidence, with `authorship: "undetermined"`. Reading a result does not use quota. Unknown IDs and IDs owned by a different account return HTTP 404.
